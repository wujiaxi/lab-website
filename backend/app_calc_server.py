#!/usr/bin/env python3
"""
Flask Backend for Spatial Data Explorer - CALCULATION SERVER VERSION
Serves H5AD data on-demand with caching and thumbnail generation

This version is optimized for the calculation server in a two-server architecture:
- Removed static page routes (served by Nginx on static server)
- Kept API routes and explorer route
- Added CORS for cross-origin requests from static server
...
# Enable CORS for requests from static server
# Update this with your actual domain
CORS(app, resources={
    r"/api/*": {"origins": ["https://pinolilab.org", "http://pinolilab.org"]},
    r"/explorer": {"origins": ["https://pinolilab.org", "http://pinolilab.org"]}
})
...
def health():
    ...
    return jsonify({
        ...
        'cache_stats': adata_cache.get_stats(),
        'data_dir': str(DATA_DIR),
        'registry_exists': REGISTRY_FILE.exists()
    })


# ============== Explorer Route (kept for server-side rendering) ==============

@app.route('/explorer')
def explorer():
    """Serve data explorer page (server-side rendered)"""
...
if __name__ == '__main__':
    print("=" * 60)
    print("Starting Calculation Server (Flask Backend)")
    print("=" * 60)
